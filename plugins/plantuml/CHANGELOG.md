...
- Legend line spacing: a `<size:5> </size>` line between entries takes the step from ~16px to ~33px, so a list of swatches stops reading as one solid block. The effect is quantized — the size number barely matters, only the number of spacer lines — and that is documented alongside
