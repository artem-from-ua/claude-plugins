...
- **Generate the link with the tool; never assemble it yourself.** Pipe the source into \`python3 ${PLUGIN_ROOT}/scripts/plantuml-encode.py --md-link "short text"\` and paste its single line of output verbatim. It encodes, self-verifies, and emits the finished \`[text](url)\` — so the encoded string is never carried through a reply by hand. To re-show a diagram that already lives in a file, get its link from \`--verify-file <file>\` rather than from memory or from an earlier message.
- **Verify before showing.** If a link reaches a reply by any other route, run \`--verify-url "<url>"\` on it first. It decodes locally — no network — and prints the diagram's own title, so a link that decodes to the wrong diagram is caught too. Exit 1 means do not show it. Saying a link was verified without having run the check is worse than not checking: it moves the error to the user.
