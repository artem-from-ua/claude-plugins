...
In an ultra effort mode, a violet `ultracode` (or `ultraplan` in plan mode) marker follows the effort:
...
- **Ultra mode** — a **violet** `ultracode` / `ultraplan` marker, shown right after effort when the
