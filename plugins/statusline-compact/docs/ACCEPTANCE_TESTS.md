...
#### 2.6 Ultra-mode Segment (violet `ultracode`/`ultraplan`)

The renderer scans `.transcript_path` for the last genuine `/effort` command and, if it selected an
ultra mode, appends a violet (`38;5;135`) `ultracode`/`ultraplan` segment right after effort. Build
...
pay xhigh "$TD/ultracode.jsonl"     | bash "$S"   # expect violet 38;5;135 "ultracode"
...
**Acceptance criteria:**
- ✅ last `/effort` = `ultracode`/`ultraplan` → violet `38;5;135` segment with that exact word, right after effort
...
- ✅ the violet SGR `38;5;135` appears **only** in an ultra case
